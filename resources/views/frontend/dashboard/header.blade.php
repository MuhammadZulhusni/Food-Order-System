         ...
         <ul class="navbar-nav ml-auto">
            ...
            <li class="nav-item dropdown">
               ...
               <div class="dropdown-menu dropdown-menu-right shadow-sm border-0">
                  ...
                  <a class="dropdown-item" href="{{ route('user.logout') }}"><i class="icofont-sign-out"></i> Logout </a>
               </div>
            </li>
            @else
            <li class="nav-item dropdown">
            ...
            @php
               // Initialize cart data for display
               $total = 0;
               $cart = session()->get('cart', []);
               $groupedCart = [];
               
               // Group cart items by client/restaurant ID
               foreach ($cart as $item) {
                   $groupedCart[$item['client_id']][] = $item;
               }
               
               // Fetch client data for all clients in the cart
               $clients = App\Models\Client::whereIn('id', array_keys($groupedCart))->get()->keyBy('id');
            @endphp
            <li class="nav-item dropdown dropdown-cart">
               <a class="nav-link dropdown-toggle" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                  <i class="fas fa-shopping-basket"></i> Cart
                  <span class="badge badge-success">{{ count($cart) }}</span>
               </a>
                  ...
                  @foreach ($groupedCart as $clientId => $items)
                     @if (isset($clients[$clientId]))
                        ...
                        <div class="dropdown-cart-top-header p-4">
                           <img class="img-fluid mr-3" alt="client logo" src="{{ asset('upload/client_images/' . $client->photo) }}">
